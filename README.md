...
5. [.env 설정](#env-설정)
6. [파일 설명](#파일-설명)
7. [macOS launchd(자동 실행) 설정](#macos-launchd자동-실행-설정)
8. [Google Drive 업로드 경로 구조](#google-drive-업로드-경로-구조)
9. [보안 권장사항](#보안-권장사항)
10. [문제 발생 시 확인 포인트](#문제-발생-시-확인-포인트)
11. [GitHub에 안전하게 업로드](#github에-안전하게-업로드)
...
- **주요 함수**:
  - `get_activities()` - 최근 활동 목록 조회
  - `download_activity()` - FIT 파일 다운로드
...
- 필수 파이썬 패키지 목록
- 주요 패키지:
  - `garminconnect==0.2.38` - Garmin API
  - `PyDrive2==1.16.5` - Google Drive API
  - `fitparse==1.2.6` - FIT 파일 파싱
  - `pandas==2.2.0` - 데이터 처리
