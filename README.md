...
6. [Zone2 분석 모드](#zone2-분석-모드)
7. [.env 설정](#env-설정)
8. [파일 설명](#파일-설명)
9. [macOS launchd(자동 실행) 설정](#macos-launchd자동-실행-설정)
10. [Google Drive 업로드 경로 구조](#google-drive-업로드-경로-구조)
11. [보안 권장사항](#보안-권장사항)
12. [문제 발생 시 확인 포인트](#문제-발생-시-확인-포인트)
...
- **FIT 분석** - 파일을 파싱해 간단한 Zone2(심박 범위) 요약 로그 출력 및 SQLite DB 저장
...
├─ requirements.txt
├─ settings.yaml
├─ analysis.db              # Zone2 분석 결과 DB (자동 생성)
├─ config/
...
│  └─ sync.log             # 실행 로그
├─ tmp/                    # 임시 FIT 파일 저장
├─ uploaded.json           # 업로드된 활동 ID 기록
├─ .sync_initialized       # 초기 동기화 완료 플래그
└─ .env                    # 환경변수 (직접 생성)
...
로컬에 저장된 FIT 파일들을 분석하여 Zone2 트레이닝 메트릭을 계산합니다.
...
### 샘플 출력
```
[2/22] 파일 activity_123.fit Zone2 분석: {'zone2_seconds': 190, 'zone2_avg_speed_kmh': 8.85, 'zone2_avg_pace_min_km': '6:47'}
```

### DB 조회 예시
```bash
sqlite3 analysis.db "SELECT filename, zone2_avg_pace_min_km FROM zone2_analysis WHERE zone2_avg_speed_kmh > 8.0"
```
